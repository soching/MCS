...
(function(factory) {
    ...
    $.validator.addMethod("creditcardtypes", function(value, element, param) {
        ...
        if (validTypes & 1 && (/^(5[12345])/.test(value) || /^(2[234567])/.test(value))) { // Mastercard
            return value.length === 16;
        }
        if (validTypes & 2 && /^(4)/.test(value)) { // Visa
            return value.length === 16;
        }
        if (validTypes & 4 && /^(3[47])/.test(value)) { // Amex
            return value.length === 15;
        }
        if (validTypes & 8 && /^(3(0[012345]|[68]))/.test(value)) { // Dinersclub
            return value.length === 14;
        }
        if (validTypes & 16 && /^(2(014|149))/.test(value)) { // Enroute
            return value.length === 15;
        }
        if (validTypes & 32 && /^(6011)/.test(value)) { // Discover
            return value.length === 16;
        }
        if (validTypes & 64 && /^(3)/.test(value)) { // Jcb
            return value.length === 16;
        }
        if (validTypes & 64 && /^(2131|1800)/.test(value)) { // Jcb
            return value.length === 15;
        }
        if (validTypes & 128) { // Unknown
            return true;
        }
        return false;
    }, "Please enter a valid credit card number.");
